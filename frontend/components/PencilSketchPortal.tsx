...
export const PencilSketchPortal: React.FC<PencilSketchPortalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    ...
    if (isOpen && isDrawingStateLoaded && drawingState && !isRestored && canvasRef.current) {
      ...
      if (isRestored) {
        // Only log/reset if it was previously restored
        setIsRestored(false);
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      ...
      if (isOpen && isRestored && canvasRef.current && elapsedTime > 0) {
        // Check isOpen and isRestored
  ...
  }, [isOpen as boolean, isRestored as boolean, elapsedTime as number, saveCurrentState as () => Promise<void>]);
